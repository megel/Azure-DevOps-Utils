...
Copy-Item -Path $PSScriptRoot -Destination "$path" -Exclude @("settings.ps1", ".gitignore", "README.md", "PublishAzureDevOpsAPIUtils.ps1") -Recurse
...
Update-ModuleManifest -Path (Join-Path $path "AzureDevOpsAPIUtils.psd1") `
                      -RootModule (Join-Path $path "AzureDevOpsAPIUtils.psm1") `
